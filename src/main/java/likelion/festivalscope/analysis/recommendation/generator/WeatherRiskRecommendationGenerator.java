...
public class WeatherRiskRecommendationGenerator implements RecommendationGenerator {
    ...
    private java.util.Optional<RecommendationDraft> rainDraft(
        ...
        if (weather.festivalCondition().spaceType() == VenueType.OUTDOOR) {
            ...
            content = switch (priority) {
                case IMMEDIATE -> String.format("최근 %d년 중 동일 시기에 강수가 발생한 해는 %d년입니다. "
                                + "야외 행사 특성상 강수 시 프로그램 운영 차질 가능성이 높습니다. "
                                + "실내 대체 공간이나 우천 시 대체 프로그램을 사전에 마련하는 것을 권장합니다.",
                        actualYears, rain.occurrenceYears());
                case REVIEW -> String.format("최근 %d년 중 동일 시기에 강수가 발생한 해는 %d년입니다. "
                                + "야외 프로그램 운영에 영향을 줄 수 있으므로 우천 시 대체 운영 방안을 준비하는 것을 권장합니다.",
                        actualYears, rain.occurrenceYears());
                case OPTIONAL -> String.format("최근 %d년 중 동일 시기에 강수가 발생한 해는 %d년입니다. "
            ...
            content = priority == RecommendationPriority.IMMEDIATE
                    ? String.format("최근 %d년 중 동일 시기에 강수가 발생한 해는 %d년입니다. "
            ...
            content = String.format("최근 %d년 중 동일 시기에 강수가 발생한 해는 %d년입니다. "
        ...
        if (spaceType == VenueType.OUTDOOR) {
            ...
            content = switch (priority) {
                case IMMEDIATE -> {
                    ...
                    yield String.format("최근 %d년 중 동일 시기에 고온이 발생한 해는 %d년이며, 평균 최고기온은 %s℃입니다. "
                                    + "장시간 야외 체류에 대비해 그늘·휴식공간, 음수 제공, 운영시간 조정 등의 대책을 마련하는 것을 권장합니다.",
                            actualYears, temperature.hotOccurrenceYears(),
                            format(temperature.averageMaxTemperature()));
                }
                case REVIEW -> String.format("최근 %d년 중 동일 시기에 고온이 발생한 해는 %d년입니다. "
                                + "야외 체류시간을 고려해 휴식공간과 운영시간 등 방문객 보호 대책을 보완하는 것을 권장합니다.",
                        actualYears, temperature.hotOccurrenceYears());
                case OPTIONAL -> String.format("최근 %d년 중 동일 시기에 고온이 발생한 해는 %d년입니다. "
            ...
            content = priority == RecommendationPriority.IMMEDIATE
                    ? String.format("최근 %d년 중 동일 시기에 고온이 발생한 해는 %d년입니다. "
            ...
            content = String.format("최근 %d년 중 동일 시기에 고온이 발생한 해는 %d년입니다. "
    ...
    private java.util.Optional<RecommendationDraft> windDraft(
        ...
        content = String.format("최근 %d년 중 동일 시기에 강풍이 발생한 해는 %d년입니다. %s",
                actualYears, wind.strongWindOccurrenceYears(), content);
